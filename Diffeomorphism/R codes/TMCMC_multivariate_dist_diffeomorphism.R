...
##############  Multivariate TMCMC results multivariate t #######################
...
for ( l in 1:L)
{
    ...
    while (i<=nsamples)
    {
	 ...
	 for (j in 1:d)
	 {
        ...
        accept_rate= min(1,exp(pdf(t(y))-pdf(t(smpl[(i-1),]))));
        w=runif(1);
        if (w<accept_rate)
	  {
            smpl[i,]=y;
...
for(d in 1:40)
{
	for(n in 1:nsamples)
	{
		...
		KSval_TMCMC_diff[n]=ks.test(Diff_TMCMC[1,,n,d],simulate.vec2)$statistic;
		KSval_TMCMC_wo_diff[n]=ks.test(Diff_TMCMC[2,,n,d],simulate.vec)$statistic;
...
title(main="KS plot comparison TMCMC");
